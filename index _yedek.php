...
$kayit_sorgu = $baglanti->prepare("SELECT * FROM surec_kayit ");

$kayit_sorgu->execute(array());
            ...
            <?php while($kayit_sorgu_cek = $kayit_sorgu->fetch(PDO::FETCH_ASSOC)) { ?>
                <tr>
                    ...
                    <td class="duzenle_btn">
                        <a class="btn btn-success" href="index.php?sayfa=is_takib&id=<?php echo $kayit_sorgu_cek['id']; ?>">Süreç Gir</a>
                        <!-- Silme butonu için ayrı form -->
...
<?php } else if(isset($_GET['sayfa']) && $_GET['sayfa'] == "is_takib") { 
...
<div id="mesaj" class="alert alert-info">Giriş tarihi seçilmedi.</div>
...
<script>
  ...
  // Her giriş değiştiğinde kontrol yap
  girisInput.addEventListener('change', () => {
    if (girisInput.value) {
      ...
      cikisInput.value = ""; // Çıkış varsa temizle
    } else {
